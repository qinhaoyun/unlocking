<template>
	<view class="content">
		...
		<view class="main" :animation="animationData">
			...
			<view class="coupon-tit" v-if="w">
				<text>点击选择优惠卷(有{{w}}张可用)</text>
				<text  @tap="go_coupon">查看更多></text>
			</view>
			<view class="coupon-box" v-if="w">
				<view class="coupon-box1" :style="{width:abc}">
					<view class="coupon" v-for="(item,index) in coupon" :key="index" @tap="check_coupon(index)">
						<image src="../../static/images/yhj.png" mode=""></image>
						<view class="num" :class="check_c===index?'color-check':''"><text class="num-money">{{item.money}}</text><text class="unit">元</text></view>
						<text class="deadline" :class="check_c===index?'color-check':''">截止到{{item.endtime}}失效</text>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		computed:{
			abc(){
				return (this.w * 242) + "rpx"
			},
			// 计算价格
			moneycount(){
				let coupon_money = ""
				if(this.w){
					coupon_money = this.coupon[this.check_c].money?this.coupon[this.check_c].money:0
				}
				// 价格小于优惠卷时返回0元
				return (this.money - coupon_money) <0? 0: this.money - coupon_money
			}
		},
		props: ["money", "balance","w","coupon","orderData"],
		methods: {
			pay(index) {
				this.checked = index
			},
			...
			goPay() {
				...
				if(this.coupon.length){
					coupon_id = this.coupon[this.check_c].id?this.coupon[this.check_c].id:'';
				}
				this.orderData.cid = coupon_id // 添加优惠卷
				this.orderData.price = this.money;
				console.log(JSON.stringify(this.orderData))
				uni.request({
					url: 'https://rg.cqlaiku.com:8811/indent/setIndent',
					data:this.orderData,
					method: 'POST',
					header: {
